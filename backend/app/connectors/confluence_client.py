...
class ConfluenceCommentNotFoundError(Exception):
    pass
...
class ConfluenceClient:
    ...
    def get_footer_comment(self, comment_id: str) -> dict[str, Any]:
        try:
            ...
            if exc.response.status_code == 404:
                raise ConfluenceCommentNotFoundError(
                    f"Confluence comment '{comment_id}' was not found."
                ) from exc
            raise
    ...
    def update_comment(self, page_id: str, comment_id: str, body: str) -> None:
        ...
        try:
            ...
            if exc.response.status_code == 404:
                raise ConfluenceCommentNotFoundError(
                    f"Confluence comment '{comment_id}' was not found for page '{page_id}'."
                ) from exc
            raise
    ...
    def _storage_to_text(self, value: str) -> str:
        ...
        text = re.sub(
            r"</(p|div|h1|h2|h3|h4|h5|h6|li|tr)>",
            "\n",
            text,
            flags=re.IGNORECASE,
        )
