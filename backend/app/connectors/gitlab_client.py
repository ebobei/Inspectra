...
class GitLabClient:
    ...
    def parse_external_id(self, external_id: str) -> tuple[str, str]:
        if '!' not in external_id:
            raise ValueError("GitLab external_id must be in format 'project_id!mr_iid' or 'group%2Fproject!mr_iid'")
    ...
    def fetch_merge_request(self, project_id: str, mr_iid: str) -> dict[str, Any]:
        ...
        response = self._request('GET', f'/api/v4/projects/{encoded_project}/merge_requests/{mr_iid}')
        return response.json()

    def create_note(self, project_id: str, mr_iid: str, body: str) -> str:
        encoded_project = quote(project_id, safe='')
        response = self._request('POST', f'/api/v4/projects/{encoded_project}/merge_requests/{mr_iid}/notes', json={'body': body})
        data = response.json()
        return str(data['id'])

    def update_note(self, project_id: str, mr_iid: str, note_id: str, body: str) -> None:
        encoded_project = quote(project_id, safe='')
        self._request('PUT', f'/api/v4/projects/{encoded_project}/merge_requests/{mr_iid}/notes/{note_id}', json={'body': body})

    def normalize_merge_request(self, mr_payload: dict[str, Any]) -> tuple[str | None, str, dict[str, Any]]:
